...
\begin{frame}[fragile]{Konfigurera Yubikeyn}
  \begin{itemize}
    ...
    \item Stäng av OTP \mintinline{shell}{$ ykman config usb --disable OTP}
    \item Aktivera touch \mintinline{shell}{$ ykman openpgp set-touch aut on}
    \item Byt pin \mintinline{shell}{$ gpg --change-pin}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{För SSH: GPG agent}
  \begin{itemize}
    \item GPG agenten hanterar nycklarna
...
\begin{frame}[fragile]{Signera commits i Git}
  \begin{itemize}
    \item Lista kända nycklar \mintinline{shell}{$ gpg --list-secret-keys --keyid-format LONG}
    \item Exportera publika nyckeln \mintinline{shell}{$ gpg --armor --export KEYID}
    \item Ladda upp på Github (eller motsv)
    \item Konfigurera git att använda en specifik nyckel \mintinline{shell}{$ git config --global user.signingkey KEYID}
